...
export function GameScreen() {
    ...
    const scale = React.useMemo(() => {
        ...
        return (value: number) => Math.round(value * ratio * 100) / 100 // Округляем до 2 знаков после запятой
    }, [wheelSize])
                            ...
                            <PanelShell scale={scale}>
                                <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap: scale(8)}}>
                                    <div style={{...getCurrencyCell(scale), background: currency==='W' ? '#ffffff' : 'linear-gradient(180deg, #9cc9ff 0%, #7db6ff 100%)', opacity: (mode === 'pyramid' && pyramidSpinCount > 0) ? 0.5 : 1, cursor: (mode === 'pyramid' && pyramidSpinCount > 0) ? 'default' : 'pointer'}} onClick={() => { if (mode === 'pyramid' && pyramidSpinCount > 0) return; setCurrency('W')}}>
                                        <div style={{fontWeight:900, fontSize: scale(18), color:'#2b66b9'}}>W</div>
                                    </div>
                                    <div style={{...getCurrencyCell(scale), background: currency==='B' ? '#ffffff' : 'linear-gradient(180deg, #9cc9ff 0%, #7db6ff 100%)', opacity: (mode === 'pyramid' && pyramidSpinCount > 0) ? 0.5 : 1, cursor: (mode === 'pyramid' && pyramidSpinCount > 0) ? 'default' : 'pointer'}} onClick={() => { if (mode === 'pyramid' && pyramidSpinCount > 0) return; setCurrency('B')}}>
                                        <div style={{fontWeight:900, fontSize: scale(18), color:'#2b66b9'}}>B</div>
                                    </div>
                                </div>
                            </PanelShell>
                            {/* Row 3: ставка */}
...
function getPlusNearWheel(scale: (v: number) => number): React.CSSProperties {
    return { 
        position:'absolute', 
        left: scale(-10), 
        bottom: scale(-40), 
        ...
        objectFit:'contain', 
        pointerEvents:'none', 
        filter:`drop-shadow(0 ${scale(4)}px ${scale(8)}px rgba(0,0,0,0.25))` 
    }
}
...
// Controls UI
function ControlRow({ children, scale }: { children: React.ReactNode, scale: (v: number) => number }){
    return <div style={{display:'grid', gridTemplateColumns:`${scale(36)}px 1fr ${scale(36)}px`, alignItems:'center', gap:scale(8)}}>{children}</div>
}
...
// Создаем функции для адаптивных стилей
