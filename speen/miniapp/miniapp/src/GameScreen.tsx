...
export function GameScreen() {
    ...
    React.useEffect(() => {
        ...
        try {
            ...
            // Ensure per-user keys exist after first login (so next switch doesn't reuse previous account)
    ...
    function scheduleNextPyramidSpin(nextSpinCount: number) {
        ...
        pyramidAutoSpinTimeoutRef.current = window.setTimeout(() => {
            ...
            try {
                ...
                if (plannedIdx >= 0) wheelRef.current.spin(plannedIdx)
                else wheelRef.current.spin()
            } catch (err) {
    ...
    function onBeforeSpin() {
        ...
        if (mode === 'pyramid') {
            ...
            // фиксируем выбранную цифру на старте серии
            ...
            // Планируем уникальные цифры для серии 3/10 (исключаем дубли)
            try {
    ...
    function onSpinResult(index: number, label: string) {
        ...
        if (pyramidBetTakenRef.current && currentPyramidCount <= pyramidMaxSpinsRef.current) {
            ...
            if (currentPyramidCount < pyramidMaxSpinsRef.current) {
                ...
                const hitIndex = newResults.indexOf(selectedNum) // -1 if miss; otherwise 0..3
        ...
        if (mode === 'normal' || mode === 'allin') {
            ...
            // ВАЖНО: при выигрыше возвращаем ставку + начисляем выигрыш по множителю.
            // Так игрок видит "итоговое начисление" (например: x2 => +3*bet, x5 => +6*bet).
                        ...
                        <ShopPanel
                            ...
                            onPurchase={(title, priceW) => {
                                ...
                                try {
                                    ...
                                    const uid = userIdRef.current
                                    if (uid) localStorage.setItem(`purchases_${uid}`, JSON.stringify(list))
                                } catch {}
                        ...
                        <WheelShopPanel
                            ...
                            onPurchase={(b) => {
                                ...
                                try {
                                    ...
                                    localStorage.setItem('bonuses_inv', JSON.stringify(inv))
                                    const uid = userIdRef.current
                                    if (uid) localStorage.setItem(`bonuses_inv_${uid}`, JSON.stringify(inv))
                                } catch {}
