...
export const ImageWheel = React.forwardRef<ImageWheelRef, ImageWheelProps>(({ size = 260, imageSrc, labels, startOffsetDeg = 0, onResult, onBeforeSpin, onSpinningChange, selectedIndex, onSelectIndex, onOpenBonuses, selectedBonusIndex, onSelectBonusSector, hideCenterButton = false, disableSelection = false, sectorBonuses = [], selectedBonusImage = null }, ref) => {
    ...
    function getPlusCenter(): { x: number, y: number } {
        ...
        const x = cx + r * Math.cos(plusAngle) + 6 // смещение правее на 6px
        const y = cy + r * Math.sin(plusAngle) + 15 // смещение вниз на 15px
        return { x, y }
    }
    ...
    function spin(toIndex?: number) {
        ...
        let forcedIndexFromBeforeSpin: number | null = null
        if (typeof onBeforeSpin === 'function') {
            const ok = onBeforeSpin()
            console.log(`[ImageWheel.spin] onBeforeSpin returned: ${ok}`)
            if (ok === false) return
            if (typeof ok === 'number' && Number.isFinite(ok)) {
                forcedIndexFromBeforeSpin = ok
            }
        ...
        const sourceIndex = (typeof toIndex === 'number' ? toIndex : (forcedIndexFromBeforeSpin != null ? forcedIndexFromBeforeSpin : undefined))
        const targetIndex = typeof sourceIndex === 'number'
            ? ((sourceIndex % labels.length) + labels.length) % labels.length
                ...
                {!isSpinning && !concealInner && sectorBonuses.length === labels.length && (
                    ...
                    >
                        {(() => {
                            ...
                            const iconSize = size * 0.09 // Увеличено с 0.08 до 0.09 для бонусов
                                    ...
                                    nodes.push(
                                        <g key={`money-icon-${i}`} transform={`translate(${x2}, ${y2}) rotate(${ang + 90})`}>
                                            ...
                                            <text
                                                ...
                                                fill="#ffffff"
                                                fontWeight={900}
                                                fontSize={iconSize * 0.28} // Увеличено с 0.18 до 0.28 для цифр
