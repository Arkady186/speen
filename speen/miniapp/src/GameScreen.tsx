...
export function GameScreen() {
    ...
    function claimNextLevel(targetLevel?: number) {
        const nextLevel = targetLevel || Math.min(50, playerLevel + 1)
        if (!isLevelRequirementMet(nextLevel)) return
        if (nextLevel <= playerLevel) return
        
        const conf = LEVELS.find(x => x.level === nextLevel)
        if (!conf) return
        
        // Выдаём награду
        saveBalances(balanceW + conf.rewardW, balanceB)
        setToast(`+${conf.rewardW} W (уровень ${nextLevel})`)
